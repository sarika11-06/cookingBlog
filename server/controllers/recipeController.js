...
exports.exploreCategoriesById = async (req, res) => {
  try {
    ...
    res.render("categories", {
      title: "Cooking Blog - categoryById",
      categoryById,
    });
  } catch (error) {
    res.status(500).send({ message: error.message || "Error Occured" });
...
//   try {
//     const res = await Recipe.deleteOne({ name: 'das' })
...
//   try {
//     const res = await Recipe.updateOne({ name: 'Vada Pav' }, { name: 'Chinese Steak' })
//     res.n; // Number of documents matched
//     res.nModified; // Number of documents modified
//   } catch (error) {
//     console.log(error)
...
//         description:
//           "Wash the mussels thoroughly, discarding any that aren't tightly closed.Trim and finely slice the spring onions, peel and finely slice the garlic. Pick and set aside the coriander leaves, then finely chop the stalks. Cut the lemongrass into 4 pieces, then finely slice the chilli.In a wide saucepan, heat a little groundnut oil and soften the spring onion, garlic, coriander stalks, lemongrass and most of the red chilli for around 5 minutes.Source: https://www.jamieoliver.com/recipes/seafood-recipes/thai-style-mussels/",
//         email: "hello@email.com",
//         ingredients: [
...
//         name: "Thai-style mussels",
//         description:
//           "Peel and crush the garlic, then peel and roughly chop the ginger. Trim the greens, finely shredding the cabbage, if using. Trim and finely slice the spring onions and chilli. Pick the herbs.Bash the lemongrass on a chopping board with a rolling pin until it breaks open, then add to a large saucepan along with the garlic, ginger and star anise.Place the pan over a high heat, then pour in the vegetable stock. Bring it just to the boil, then turn down very low and gently simmer for 30 minutes.Source: https://www.jamieoliver.com/recipes/vegetables-recipes/asian-vegetable-broth/",
//         email: "hello@email.com",
//         ingredients: [
//           "3 cloves of garlic",
//           "5cm piece of ginger",
...
//         description:
//           "Preheat the oven to 170ºC/325ºF/gas 3 and line 2 baking sheets with greaseproof paper.Combine the cocoa powder with a little warm water to form a paste, then add to a bowl with the remaining whoopie ingredients. Mix into a smooth, slightly stiff batter.Spoon equal-sized blobs, 2cm apart, onto the baking sheets, then place in the hot oven for 8 minutes, or until risen and cooked through.Cool for a couple of minutes on the sheets, then move to a wire rack to cool completely.Once the whoopies are cool, spread ½ a teaspoon of dulce de leche on the flat sides.Peel and slice the bananas, then top half the pies with 2 slices of the banana.Sandwich together with the remaining halves, and dust with icing sugar and cocoa powder.\n    Source: https://www.jamieoliver.com/recipes/chocolate-recipes/chocolate-amp-banoffee-whoopie-pies/",
//         email: "hello@email.com",
//         ingredients: [
//           "3 spring onions",
...
//         description:
//           "Drain the juices from the tinned fruit into a bowl, add the soy and fish sauces, then whisk in 1 teaspoon of cornflour until smooth. Chop the pineapple and peaches into bite-sized chunks and put aside.Pull off the chicken skin, lay it flat in a large, cold frying pan, place on a low heat and leave for a few minutes to render the fat, turning occasionally. Once golden, remove the crispy skin to a plate, adding a pinch of sea salt and five-spice.Meanwhile, slice the chicken into 3cm chunks and place in a bowl with 1 heaped teaspoon of five-spice, a pinch of salt, 1 teaspoon of cornflour and half the lime juice. Peel, finely chop and add 1 clove of garlic, then toss to coat.\n    Source: https://www.jamieoliver.com/recipes/chicken-recipes/tom-daley-s-sweet-sour-chicken/",
//         email: "hello@email.com",
//         ingredients: [
...
//         description:
//           "Remove the butter from the fridge and allow to soften.Preheat a griddle pan until really hot.Butter the rolls on both sides and grill on both sides until toasted and lightly charred (keep an eye on them so they don’t burn).Trim and dice the celery, chop the lobster meat and combine with the mayonnaise. Season with sea salt and black pepper to taste.Open your warm grilled buns, shred and pile the lettuce inside each one and top with the lobster mixture. Serve immediately.\n    Source: https://www.jamieoliver.com/recipes/fruit-recipes/key-lime-pie/",
//         email: "hello@email.com",
//         ingredients: [
//           "85 g butter",
//           "6 submarine rolls",
